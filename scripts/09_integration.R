...
if (length(args) < 2) {
    stop(
        "Usage: Rscript 09_integration.R",
...
message(sprintf(
    "[peaks] Peaks in promoters (±%dkb TSS): %d",
    promoter_window / 1000, nrow(promoter_peaks)
))

message("[id] Converting Entrez → Ensembl for ChIP-seq peaks")
promoter_peaks$ensembl_id <- mapIds(
    org.Hs.eg.db,
    keys      = as.character(promoter_peaks$geneId),
    column    = "ENSEMBL",
    keytype   = "ENTREZID",
    multiVals = "first"
)

n_mapped <- sum(!is.na(promoter_peaks$ensembl_id))
message(sprintf("[id] %d / %d peaks mapped to Ensembl ID", n_mapped, nrow(promoter_peaks)))

promoter_peaks_mapped <- promoter_peaks[!is.na(promoter_peaks$ensembl_id), ]
...
direct_targets <- merge(
    promoter_peaks_mapped, degs_sig,
    by.x = "ensembl_id", by.y = "gene_id",
    all = FALSE
)

indirect_targets <- degs_sig[
    !degs_sig$gene_id %in% promoter_peaks_mapped$ensembl_id,
]

bound_no_change <- promoter_peaks_mapped[
    !promoter_peaks_mapped$ensembl_id %in% degs_sig$gene_id,
]

direct_activated <- direct_targets[!is.na(direct_targets$log2FoldChange) &
    direct_targets$log2FoldChange > 0, ]
direct_repressed <- direct_targets[!is.na(direct_targets$log2FoldChange) &
    direct_targets$log2FoldChange < 0, ]
...
gene_sym_col <- if ("SYMBOL" %in% colnames(direct_targets)) "SYMBOL" else if ("geneSymbol" %in% colnames(direct_targets)) "geneSymbol" else NULL
...
if (!is.null(score_col) && nrow(direct_targets) > 1L) {
    cor_res <- tryCatch(
        cor.test(direct_targets[[score_col]], abs(direct_targets$log2FoldChange),
            method = "spearman"
        ),
    ...
    save_pdf(
        {
            ...
            if (!is.null(gene_sym_col)) {
                label_df <- plot_df[!is.na(plot_df[[gene_sym_col]]) &
                    plot_df[[gene_sym_col]] %in% key_genes, ]
                if (nrow(label_df) > 0L) {
                    p <- p + geom_text_repel(
...
tryCatch(
    {
        ...
        venn_list <- list(
            Promoter_Peaks = as.character(promoter_peaks_mapped$ensembl_id),
            DEGs           = as.character(degs_sig$gene_id)
        )
        save_pdf(
...
tryCatch(
    {
        ...
        edges_act <- data.frame(
            from = chip_base,
            to = as.character(direct_activated$ensembl_id),
            type = "activation",
        ...
        edges_rep <- data.frame(
            from = chip_base,
            to = as.character(direct_repressed$ensembl_id),
            type = "repression",
        ...
        if (nrow(edges_all) > 0L) {
            ...
            igraph::V(g)$color <- ifelse(igraph::V(g)$name == chip_base, "red",
                ifelse(igraph::V(g)$name %in% edges_act$to,
                    "green", "blue"
                )
            )
...
if (!is.null(gene_sym_col)) {
    validated <- direct_targets[
        !is.na(direct_targets[[gene_sym_col]]) &
            direct_targets[[gene_sym_col]] %in% known_p53_targets,
        c(
            "ensembl_id", "geneId", gene_sym_col, "log2FoldChange", "padj",
    ...
    if (nrow(validated) > 0L) {
        message(paste(" ", validated[[gene_sym_col]], collapse = "\n"))
