...
get_sym_col <- function(df) {
    found <- intersect(c("SYMBOL", "SYMBOL.x", "geneSymbol", "symbol"), colnames(df))
...
if (!is.null(targets_long)) {
    p1a <- targets_long %>%
        ...
        scale_fill_manual(values = c(activated = PALETTE$activated, repressed = PALETTE$repressed)) +
        labs(title = "A  Direct p53 targets per dataset", x = NULL, y = "Genes", fill = NULL) +
        THEME_PUB +
    ...
    if (!is.null(conservation_df) && "n_runs" %in% colnames(conservation_df)) {
        p1b <- ggplot(conservation_df, aes(x = n_runs)) +
            geom_histogram(binwidth = 1, fill = PALETTE$green, color = "white", boundary = 0.5) +
            scale_x_continuous(breaks = seq_len(max(conservation_df$n_runs, na.rm = TRUE))) +
            labs(title = "B  Target conservation across runs", x = "Number of runs", y = "Genes") +
            THEME_PUB

        save_pdf(print(p1a + p1b + plot_layout(widths = c(1.4, 1))),
...
if (!is.null(targets_long)) {
    ...
    save_pdf(print(p2), file.path(fig_dir, "fig2_volcano.pdf"),
...
if (!is.null(cox_df)) {
    ...
    p3 <- ggplot(forest_df, aes(x = hr, y = gene, xmin = hr_lo95, xmax = hr_hi95, color = sig)) +
    ...
    save_pdf(print(p3), file.path(fig_dir, "fig3_survival_forest.pdf"),
...
if (!is.null(cat_enrich)) {
    p4 <- cat_enrich %>%
        ...
        mutate(category = factor(category, levels = category), sig = pvalue < 0.05) %>%
...
message("[fig5] Heatmap: conserved targets across runs")

if (!is.null(conservation_df) && !is.null(targets_long)) {
    run_cols <- setdiff(colnames(conservation_df), c("symbol", "n_runs", "known_p53"))

    conserved_syms <- conservation_df$symbol[
        conservation_df$n_runs == length(run_cols) & !is.na(conservation_df$symbol)
    ]
